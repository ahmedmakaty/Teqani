...
public class GenerateOTPUseCase extends CompletableUseCase<String> {

    InternetConnectionUseCase internetConnectionUseCase;
    UserRepository userRepository;

    @Inject
    public GenerateOTPUseCase(ThreadExecutor threadExecutor, PostExecutionThread postExecutionThread, InternetConnectionUseCase internetConnectionUseCase, UserRepository userRepository) {
    ...
    protected Completable buildUseCaseObservable(String s) {
        return internetConnectionUseCase.buildUseCaseObservable(null).flatMapCompletable(status -> {
            if (status) {
                return userRepository.generateOTP(s);
            } else {
